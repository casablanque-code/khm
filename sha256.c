...
static void sha256_transform(khm_sha256_ctx *ctx, const uint8_t *block) {
...
void khm_sha256_update(khm_sha256_ctx *ctx, const uint8_t *data, size_t len) {
    ...
    if (partial) {
        ...
        memcpy(ctx->buf + partial, data, space);
...
void khm_sha256_final(khm_sha256_ctx *ctx, uint8_t digest[32]) {
    ...
    khm_sha256_update(ctx, pad, padlen);

    /* big-endian bit count */
    uint8_t bc[8];
    bc[0] = (uint8_t)(bits >> 56); bc[1] = (uint8_t)(bits >> 48);
    bc[2] = (uint8_t)(bits >> 40); bc[3] = (uint8_t)(bits >> 32);
    bc[4] = (uint8_t)(bits >> 24); bc[5] = (uint8_t)(bits >> 16);
    bc[6] = (uint8_t)(bits >>  8); bc[7] = (uint8_t)(bits);
    khm_sha256_update(ctx, bc, 8);
...
void khm_sha256(const uint8_t *data, size_t len, uint8_t digest[32]) {
    khm_sha256_ctx ctx;
    khm_sha256_init(&ctx);
    khm_sha256_update(&ctx, data, len);
    khm_sha256_final(&ctx, digest);
}
